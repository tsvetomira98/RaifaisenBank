...
if ( ! function_exists( 'realestate_base_posted_on' ) ) :
	...
	function realestate_base_posted_on() {
		...
		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			...
			comments_popup_link( esc_html__( 'Leave a comment', 'realestate-base' ), esc_html__( '1 Comment', 'realestate-base' ), esc_html__( '% Comments', 'realestate-base' ) );
			echo '</span>';
		}
		...
		if ( 'post' === get_post_type() ) {
			...
			$categories_list = get_the_category_list( esc_html__( ', ', 'realestate-base' ) );
			if ( $categories_list && realestate_base_categorized_blog() ) {
				printf( '<span class="cat-links">%1$s</span>', $categories_list ); // WPCS: XSS OK.
			}

			/* Translators: used between list items, there is a space after the comma. */
			$tags_list = get_the_tag_list( '', esc_html__( ', ', 'realestate-base' ) );
			if ( $tags_list ) {
...
if ( ! function_exists( 'realestate_base_entry_footer' ) ) :
	...
	function realestate_base_entry_footer() {
		...
		edit_post_link( esc_html__( 'Edit', 'realestate-base' ), '<span class="edit-link">', '</span>' );
	}
endif;
