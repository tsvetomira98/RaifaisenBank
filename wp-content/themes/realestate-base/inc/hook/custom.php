...
if ( ! function_exists( 'realestate_base_skip_to_content' ) ) :
	...
	function realestate_base_skip_to_content() {
		?><a class="skip-link screen-reader-text" href="#content"><?php esc_html_e( 'Skip to content', 'realestate-base' ); ?></a><?php
	}
endif;
...
if ( ! function_exists( 'realestate_base_footer_copyright' ) ) :
	...
	function realestate_base_footer_copyright() {
		...
		// Powered by content.
		$powered_by_text = sprintf( esc_html__( 'Realestate Base by %s', 'realestate-base' ), '<a target="_blank" rel="designer" href="https://wenthemes.com/">' . esc_html__( 'WEN Themes', 'realestate-base' ) . '</a>' );
...
if ( ! function_exists( 'realestate_base_customize_page_title' ) ) :
	...
	function realestate_base_customize_page_title( $title ) {
		...
		elseif ( is_search() ) {
			$title = sprintf( __( 'Search Results for: %s', 'realestate-base' ),  get_search_query() );
		}
		elseif ( is_404() ) {
			$title = __( '404!', 'realestate-base' );
		}
		return $title;
	}
endif;
