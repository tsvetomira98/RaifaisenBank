...
function realestate_base_customize_register( $wp_customize ) {
	...
	$wp_customize->get_control( 'background_color' )->description = __( 'Note: Background Color is applicable only if no image is set as Background Image.', 'realestate-base' );

	// Register custom section types.
	$wp_customize->register_section_type( 'Realestate_Base_Customize_Section_Upsell' );

	// Register sections.
	$wp_customize->add_section(
		new Realestate_Base_Customize_Section_Upsell(
			$wp_customize,
			'theme_upsell',
			array(
				'title'    => esc_html__( 'Realestate Base Pro', 'realestate-base' ),
				'pro_text' => esc_html__( 'Buy Pro', 'realestate-base' ),
				'pro_url'  => 'https://themepalace.com/downloads/realestate-base-pro/',
				'priority' => 1,
			)
		)
	);
 ...
 * @since 1.0.0
 */
function realestate_base_customizer_control_scripts() {

	$min = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';

	wp_enqueue_script( 'realestate-base-customize-controls', get_template_directory_uri() . '/js/customize-controls' . $min . '.js', array( 'customize-controls' ) );

	wp_enqueue_style( 'realestate-base-customize-controls', get_template_directory_uri() . '/css/customize-controls' . $min . '.css' );

}

add_action( 'customize_controls_enqueue_scripts', 'realestate_base_customizer_control_scripts', 0 );
